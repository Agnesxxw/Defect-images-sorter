...
class MODEL:
    ...
    def Lenet(width=64,height=64,depth=3,weight_decay=0.0005,classNum=0):
        ...
        # 第一段
        model.add(Conv2D(40, (3, 3),padding="same",input_shape=inputShape,kernel_regularizer=regularizers.l2(weight_decay)))
        model.add(Activation("relu"))
        model.add(Conv2D(40, (3, 3), padding="same", kernel_regularizer=regularizers.l2(weight_decay)))
        model.add(Activation("relu"))
        model.add(Conv2D(40, (3, 3), padding="same",kernel_regularizer=regularizers.l2(weight_decay)))
        model.add(Activation("relu"))
        ...
        # 第二段
        model.add(Conv2D(70, (3, 3), padding="same",kernel_regularizer=regularizers.l2(weight_decay)))
        model.add(Activation("relu"))
        model.add(Conv2D(70, (3, 3), padding="same",kernel_regularizer=regularizers.l2(weight_decay)))
        model.add(Activation("relu"))
        model.add(Conv2D(70, (3, 3), padding="same", kernel_regularizer=regularizers.l2(weight_decay)))
...
def train(aug, trainX, trainY, testX, testY):
    ...
    model = MODEL.Lenet(norm_size, norm_size, depth, 0, classNum=CLASS_NUM)
    model.summary()
    # model= keras.models.load_model('ResNet50.hdf5')
    from keras.callbacks import ReduceLROnPlateau
    reduce_lr = ReduceLROnPlateau(monitor='val_acc', patience=50, mode='auto')
    adam = Adam(lr=INIT_LR, decay=INIT_LR / EPOCHS)
    # sgd = SGD(lr=INIT_LR,decay=INIT_LR / EPOCHS, momentum=0.9)
    ...
    # 权值，每层输出值的分布直方图
    H = model.fit_generator(aug.flow(trainX, trainY, batch_size=BS),callbacks=[model_checkpoint,tb_cb,reduce_lr],
